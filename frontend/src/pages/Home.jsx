import { Link } from "react-router-dom";

const Home = () => {
  return (
    <div className="bg-[#FAF8F5] text-[#3E3A36]">
      {/* HERO SECTION */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20 lg:py-24 grid gap-10 lg:gap-16 md:grid-cols-2 items-center">
        <div className="space-y-6 text-center md:text-left">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold leading-tight">
            Compassionate Care, <br />
            <span className="text-[#8B6F47]">Trusted Doctors</span>
          </h1>
          <p className="mt-2 text-base sm:text-lg text-[#5A554F]">
            Book appointments with experienced doctors and receive
            high-quality medical care in a calm and professional environment.
          </p>

          <div className="mt-6 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center md:justify-start">
            <Link
              to="/book-appointment"
              className="bg-[#8B6F47] text-white px-6 py-3 rounded-full text-sm sm:text-base text-center hover:bg-[#7A5F3E] transition"
            >
              Book Appointment
            </Link>
            <Link
              to="/services"
              className="border border-[#8B6F47] text-[#8B6F47] px-6 py-3 rounded-full text-sm sm:text-base text-center hover:bg-[#D8CFC4] transition"
            >
              Our Services
            </Link>
          </div>
        </div>

        <div className="flex justify-center md:justify-end">
          <img
            src="https://img.freepik.com/free-photo/woman-doctor-wearing-lab-coat-with-stethoscope-isolated_1303-29791.jpg?semt=ais_hybrid&w=740&q=80"
            alt="Clinic"
            className="w-full max-w-sm sm:max-w-md rounded-2xl shadow-lg object-cover"
          />
        </div>
      </section>

      {/* STATS SECTION */}
      <section className="bg-[#D8CFC4] py-12 sm:py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-6 text-center">
          <div>
            <h3 className="text-2xl sm:text-3xl font-bold text-[#8B6F47]">15+</h3>
            <p className="mt-1 text-sm sm:text-base">Expert Doctors</p>
          </div>
          <div>
            <h3 className="text-2xl sm:text-3xl font-bold text-[#8B6F47]">10k+</h3>
            <p className="mt-1 text-sm sm:text-base">Happy Patients</p>
          </div>
          <div>
            <h3 className="text-2xl sm:text-3xl font-bold text-[#8B6F47]">20+</h3>
            <p className="mt-1 text-sm sm:text-base">Medical Services</p>
          </div>
          <div>
            <h3 className="text-2xl sm:text-3xl font-bold text-[#8B6F47]">24/7</h3>
            <p className="mt-1 text-sm sm:text-base">Support</p>
          </div>
        </div>
      </section>

      {/* SERVICES */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
        <h2 className="text-2xl sm:text-3xl font-bold text-center">
          Our Medical Services
        </h2>
        <p className="text-center text-[#5A554F] mt-3 text-sm sm:text-base">
          Comprehensive healthcare services under one roof
        </p>

        <div className="mt-10 sm:mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {[
            "General Consultation",
            "Dental Care",
            "Cardiology",
            "Pediatrics",
            "Orthopedics",
            "Neurology",
          ].map((service, i) => (
            <div
              key={i}
              className="bg-white p-5 sm:p-6 rounded-2xl shadow border border-[#E6DED5] hover:shadow-lg transition"
            >
              <h3 className="text-lg sm:text-xl font-semibold text-[#8B6F47]">
                {service}
              </h3>
              <p className="mt-3 text-sm text-[#5A554F] leading-relaxed">
                Professional medical care provided by experienced specialists.
              </p>
            </div>
          ))}
        </div>
      </section>

      {/* WHY CHOOSE US */}
      <section className="bg-white py-14 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid gap-10 lg:gap-12 md:grid-cols-2 items-center">
          <div className="flex justify-center md:justify-start">
            <img
              src="data:image/jpeg;base64,"
              alt="Doctors"
              className="w-full max-w-sm sm:max-w-md rounded-2xl shadow-lg object-cover"
            />
          </div>
          <div className="text-center md:text-left">
            <h2 className="text-2xl sm:text-3xl font-bold">
              Why Choose Our Clinic?
            </h2>
            <ul className="mt-6 space-y-3 sm:space-y-4 text-[#5A554F] text-sm sm:text-base">
              <li>✔ Experienced & certified doctors</li>
              <li>✔ Easy online appointment booking</li>
              <li>✔ Modern equipment & facilities</li>
              <li>✔ Patient-first approach</li>
            </ul>
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="bg-[#8B6F47] py-12 sm:py-16 text-center text-white px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl sm:text-3xl font-bold">
          Your Health Is Our Priority
        </h2>
        <p className="mt-3 text-base sm:text-lg opacity-90 max-w-xl mx-auto">
          Book an appointment today and get expert medical care.
        </p>
        <Link
          to="/book-appointment"
          className="
            inline-block mt-6
            bg-white text-[#8B6F47]
            px-7 sm:px-8 py-3 rounded-full font-medium text-sm sm:text-base
            hover:bg-[#FAF8F5] hover:scale-105
            transition-transform duration-300 shadow-sm hover:shadow-md
          "
        >
          Book Appointment
        </Link>
      </section>
    </div>
  );
};

export default Home;